...
|---|---|---|---|
| `mention_id` | yes | `M-YYYYMMDD-NNN` | `YYYYMMDD` is the week's Monday; `NNN` sequential within the week |
| `week_of` | yes | `YYYY-MM-DD` | Must be a **Monday**. The reporting week the item counts toward |
...
| Column | Format | Notes |
|---|---|---|
| `week_of` | `YYYY-MM-DD` (Monday) | The week the snapshot belongs to |
